...
export default function JobApplicationManagementPage() {
    const { user } = useUser()
    ...
    const stats = [
        ...
        { label: "Shortlisted", value: applications.filter(a => a.application_status === "shortlisted").length, icon: Briefcase, color: "bg-purple-100 text-purple-600" },
    ...
    const columns = getColumns((application) => {
        router.push(`/job-listings/view/${application.listing_id}/applications/${application.application_id}`)
    })
                    ...
                    <Card className="rounded-lg border border-slate-200 shadow-sm">
                        ...
                        <CardContent className="px-5 py-3">
                            <DataTable
                                columns={columns}
                                data={applications}
                                searchPlaceholder="Search applicants or jobs..."
