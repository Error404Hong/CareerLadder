...
export const getColumns = (
    onView: (application: JobApplication) => void
        ...
        {
            accessorKey: "title",
            header: () => <span className="text-sm font-medium text-left block">Job</span>,
        ...
        },
        {
            ...
            header: () => <span className="text-sm font-medium text-left block">Expected Salary</span>,
            cell: ({ row }) => (
                <span className="text-sm text-slate-500">
                    {row.original.expected_salary ? `RM ${Number(row.original.expected_salary).toLocaleString()}` : "—"}
                </span>
            )
        },
        {
            accessorKey: "availability",
            header: () => <span className="text-sm font-medium text-left block">Availability</span>,
            cell: ({ row }) => <span className="text-sm text-slate-500">{row.original.availability ?? "—"}</span>
        },
        {
