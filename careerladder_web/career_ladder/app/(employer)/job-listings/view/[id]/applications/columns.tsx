...
export const getColumns = (
    ...
    rankingMap?: Map<string, { score: number; reasoning: string }>
        ...
        },
        {
            ...
            cell: ({ row }) => (
                <span className="text-sm text-slate-500">RM {Number(row.getValue("expected_salary")).toLocaleString()}</span>
            )
        },
        {
            accessorKey: "availability",
            header: () => <span className="text-sm font-medium text-left block">Availability</span>,
            cell: ({ row }) => <span className="text-sm text-slate-500">{row.getValue("availability")}</span>
        },
        {
