...
import { type ProjectApplication } from "../../columns/projectColumn"
import { getProjectById, getProjectApplications } from "@/app/api/project"
...
import { CalendarDays, Clock, DollarSign, Video, Folder } from "lucide-react"
...
export default function ViewProjectApplicationDetails() {
    const { user } = useUser()
    ...
    useEffect(() => {
        ...
        const fetchDetails = async () => {
            try {
                ...
                else toast.error("Failed to fetch project details")

                if (appsRes.success) {
                    const found = (appsRes.data as ProjectApplication[]).find(a => a.listing_id === projectId)
                    if (found) setApplication(found)
                }

                try {
                    const meetingRes = await getApplicantMeetingById(projectId, "project", user.id)
        ...
        <div className="min-h-screen bg-slate-100">
                ...
                <div className="max-w-6xl mx-auto px-6 py-6">
                    <Breadcrumb className="mb-4">
                        <BreadcrumbList>
                            ...
                            <BreadcrumbSeparator />
                            <BreadcrumbItem><BreadcrumbPage>{project?.title ?? "Project Details"}</BreadcrumbPage></BreadcrumbItem>
                        </BreadcrumbList>
                    </Breadcrumb>

                    <div className="flex items-start justify-between gap-4 flex-wrap">
                        <div className="flex flex-col gap-1">
                            <h1 className="text-2xl font-bold text-[#0f172a] tracking-tight">{project?.title}</h1>
            ...
            <div className="max-w-6xl mx-auto px-6 py-8">
                <div className="grid grid-cols-1 lg:grid-cols-5 gap-5">
                    ...
                    {/* Left (40%) — Status + Project details */}
                    <div className="lg:col-span-2 flex flex-col gap-5">
                        ...
                        <Card className="rounded-2xl border border-slate-200 shadow-sm">
                            <CardHeader>
                                <CardTitle className="text-base font-semibold text-[#0f172a]">Project Details</CardTitle>
                            </CardHeader>
                            <Separator />
                            <CardContent className="flex flex-col gap-4">
                                <div className="flex flex-wrap gap-2">
                                    ...
                                    <span className={`text-xs font-medium px-2.5 py-1 rounded-full ${project?.status === "open" ? "bg-green-50 text-green-600 border border-green-100" : "bg-red-50 text-red-500 border border-red-100"}`}>
                                ...
                                <div className="flex flex-col gap-3">
                                    <div className="flex flex-col gap-0.5">
                                        <p className="text-[11px] text-slate-400 uppercase tracking-widest">Project </p>
                                    ...
                                    <div className="flex flex-col gap-0.5">
                                        <p className="text-[11px] text-slate-400 uppercase tracking-widest">Monthly Allowance</p>
                                        <div className="flex items-center gap-1.5 text-sm text-slate-600 font-medium">
                                            <DollarSign size={13} className="text-slate-400" />
                                            RM {Number(project?.allowance).toLocaleString()} /mo
                                        </div>
                                    </div>
                                    <div className="flex flex-col gap-0.5">
                                        <p className="text-[11px] text-slate-400 uppercase tracking-widest">Period</p>
                                        <div className="flex items-center gap-1.5 text-sm text-slate-600">
                                            <CalendarDays size={13} className="text-slate-400" />
                                            {project?.start_date
                                                ? new Date(project.start_date).toLocaleDateString("en-MY", { month: "short", year: "numeric" })
                                                : "—"} —{" "}
                                            {project?.end_date
                                                ? new Date(project.end_date).toLocaleDateString("en-MY", { month: "short", year: "numeric" })
                                                : "—"}
                                        </div>
                                    </div>
                                </div>
