        ...
        <div className="flex flex-col gap-4">
            ...
            <div className="flex items-center justify-between">
                ...
                <Pagination className="w-fit mx-0">
                    <PaginationContent>
                        <PaginationItem>
                            <PaginationPrevious onClick={() => table.previousPage()} className={!table.getCanPreviousPage() ? "pointer-events-none opacity-40 cursor-not-allowed" : "cursor-pointer"} />
                        </PaginationItem>
                        {Array.from({ length: table.getPageCount() }, (_, i) => {
                            const page = i + 1
                            const currentPage = table.getState().pagination.pageIndex + 1
                            const isFirst = i === 0, isLast = i === table.getPageCount() - 1
                            const isNearCurrent = Math.abs(currentPage - page) <= 1
                            if (isFirst || isLast || isNearCurrent) {
                                return (
                                    <PaginationItem key={i}>
                                        <PaginationLink onClick={() => table.setPageIndex(i)} isActive={currentPage === page} className="cursor-pointer">{page}</PaginationLink>
                                    </PaginationItem>
                                )
                            }
                            if (Math.abs(currentPage - page) === 2) return <PaginationItem key={i}><PaginationEllipsis /></PaginationItem>
                            return null
                        })}
                        <PaginationItem>
                            <PaginationNext onClick={() => table.nextPage()} className={!table.getCanNextPage() ? "pointer-events-none opacity-40 cursor-not-allowed" : "cursor-pointer"} />
                        </PaginationItem>
                    </PaginationContent>
                </Pagination>
            </div>
