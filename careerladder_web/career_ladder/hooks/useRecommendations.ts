...
export function useRecommendations(
    ...
    const fetchRecommendations = useCallback(
        async (force = false) => {
            ...
            try {
                const [profileRes, skillsRes, experienceRes, educationRes] =
                    await Promise.all([
                ...
                const student = {
                    skills:
                        skillsRes.data?.map((s: { name: string }) => s.name) ??
                        [],
    ...
    // Skipped entirely when autoFetch is false (listing pages use the refresh button instead)
    const hasListings =
        jobs.length > 0 || projects.length > 0 || trainings.length > 0;
    useEffect(() => {
        if (!autoFetch || !user || !hasListings || hasFetched.current) return;
        hasFetched.current = true;
        fetchRecommendations();
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [autoFetch, user?.id, hasListings]);
